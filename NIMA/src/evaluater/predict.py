...
def main(base_model_name, weights_file, image_source, predictions_file, img_format='jpg'):
    ...
    data_generator = TestDataGenerator(samples, image_dir, 64, 5, nima.preprocessing_function(),
...
#        sample['mean_score_prediction'] = calc_mean_score(predictions[i])
        sample['mean_score_prediction'] = mean_score(predictions[i])
        if sample['mean_score_prediction'] < 3.0:
            sample['label'] = 0
        else:
            sample['label'] = 1

    print(json.dumps(samples, indent=2))
    print('rater0: {0}'.format(predictions[0]))
    print('rater1: {0}'.format(predictions[1]))
