...
def train(
    ...
    job_dir,
    img_format='jpg',
    existing_weights='../model_hdf5/AVA/weights_mobilenet_0.079.hdf5',
    ...
    **kwargs
    ...
    if existing_weights is not None:
        ...
        print('existing_weights: {0}'.format(existing_weights))
...
#
#    history = nima.nima_model.fit_generator(
...
#    df = pd.DataFrame({'loss': loss, 'val_loss': val_loss, 'epochs': epochs})
#    df.to_csv('loss_process1.csv', index=False)
    ...
    epochs = range(1, len(loss)+1)
    df = pd.DataFrame({'loss': loss, 'val_loss': val_loss, 'epochs': epochs})
    df.to_csv('loss_process.csv', index=False)
