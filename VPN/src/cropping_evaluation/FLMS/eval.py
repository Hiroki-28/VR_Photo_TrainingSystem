...
def main():
    ...
    #parser.add_argument('--model_path', type=str, default='../../model_pth/CPC/VPN_20221210.pth')
    parser.add_argument('--model_path', type=str, default='../../model_pth/VRPhoto/VPN_20221221.pth')
    parser.add_argument('--anchorbox_path', type=str, default='../../config/anchor_square_500.pkl')
    args = parser.parse_args()
    
    # load anchor
    pdefined_anchors = pd.read_pickle(args.anchorbox_path)
    # load FLMS data
    ...
    csvfile_savedir = get_dir('./output/VRPhoto')
    df = pd.DataFrame({'IOU_mean': [iou_mean], 'DispError_mean': [disp_mean], 'OffsetError_mean': [offset_mean]}).T
    #df.to_csv(f"{csvfile_savedir}/VPN_20221221.csv", header=None)
