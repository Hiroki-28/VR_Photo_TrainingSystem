...
def calc_probability(patche):
    ...
    # prob = color_harmony_net(patche[0], patche[1], patche[2], patche[3], patche[4]).detach()  # without  label
    prob = color_harmony_net(patche[0], patche[1], patche[2], patche[3], patche[4]).data.cpu().numpy()[0][0]
    return prob

# def calc_probability(patche, color_harmony_net, result):
#     prob = color_harmony_net(patche[0], patche[1], patche[2], patche[3], patche[4]).data.cpu().numpy()[0][0]
#     result.append(prob)
...
def main():
    ...
    #     result = manager.list()
    #     processes = []
    #     for patche in patches_list:
    #         p = Process(target=calc_probability, args=(patche, color_harmony_net, result))
    #         p.start()
    #         processes.append(p)
    #     for p in processes:
    #         p.join()
