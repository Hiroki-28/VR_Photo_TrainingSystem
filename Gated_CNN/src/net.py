...
class ResNet(nn.Module):
    def __init__(self):
        ...
        self.resnet_1 = nn.Sequential(
                BasicBlock(3,64,1),   #32,3,16,16 -> 32,64,16,16
                BasicBlock(64,64,1))
        self.resnet_2 = nn.Sequential(
                BasicBlock(64,128,2),  #32,64,16,16 -> 32,128,8,8
                BasicBlock(128,128,1))
        self.avgpool = nn.AdaptiveAvgPool2d((1,1)) # 32,128,8,8 -> 32,128,1,1
        self.flatten = nn.Flatten() # 32,128,1,1 -> 32,128 (batchsize + 3dim -> batchsize + 1dim)

    def forward(self, x):
        out = self.resnet_1(x)
        out = self.resnet_2(out)
        out = self.avgpool(out)
        out = self.flatten(out)  #0番目のバッチサイズは、平滑化されないようになっている
        return out
"""
...
class ColorHarmonyNet(nn.Module):
    ...
    def forward(self, c, n1, n2, n3, n4):
        #中心パッチの特徴
